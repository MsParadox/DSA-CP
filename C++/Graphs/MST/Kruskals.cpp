...
void Union(vector<int> &parent ,vector<int> &rank ,int a,int b){//O(log*N)
    a = find(parent,a);
    b = find(parent,b);
    if(a==b) return;
...
ll kruskals(vector<Edge> &input ,int n ,int e){
    ...
    while(edgeCount<n-1 and i<input.size()){
        ...
        i++;//does'nt matter u picked the last edge or not ,we still need to go to next edge
    }
  return ans;
}
...
// 5 5
// 0 1 4 this edge will erase to remove cycle
// 1 2 1
// 2 0 3
// 2 3 8
// 3 4 2
