...
//The code for forground and background color are:
...
//reset                0 (everything back to reset)
//bold/bright          1 (often a brighter shade of the color)
//underline            4
...
const string magneta("\033[0;35m");
...
const string magneta_bg("\033[0;45m");
...
void printPicture(char** pic ,int n,int m){
    for (int i=0;i<n;i++){
        for (int j = 0; j <m; j++)
        {//char block =-2
...
void fillColor(char** pic , int i,int j,int n,int m){
    if(i<0 or j<0 or i>=n or j>=m or pic[i][j]=='R' or pic[i][j]=='#') return;
    pic[i][j]='R';
    system("clear");
    printPicture(pic,n,m);
    sleep();
